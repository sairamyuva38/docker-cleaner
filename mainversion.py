...
#print(unique_images_startswith_acceldata)
unique_images_endswith_acceldata = set(l3)
#print(unique_images_endswith_acceldata)
...
def matchtagsandseperate(images_dict):
    for imagename in images_dict:
        ...
        version_all = [v for v in image_to_tagmap[imagename] if not (re.match(r'(\d{2})(\d{2})(\d{2})',v) and re.match(r'(\d).(\d).(\d)-(\d+)',v)) ]
        version_list =[x for x in version_all if (re.match (r'^[0-9]{1}.[0-9]{1}.[0-9]+-[0-9]+?$',x) or re.match (r'^[0-9]{1}\.[0-9]{1}\.[0-9]+$',x))]
        version_tuple = tuple(version_list)
        #print(version_tuple)
        image2tag[version_tuple]=imagename
 
         
        os.system("python untag_version.py {0}".format(image2tag[version_tuple]))
